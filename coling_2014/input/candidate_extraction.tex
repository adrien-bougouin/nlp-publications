...
\label{sec:definition_of_candidate_keyphrases}
  ...
  \label{subsec:keyphrase_analysis}
    ...
    Indeed, an adjective such as ``huge'' is almost never a relevant keyphrase
    modifier, but (for the article \textit{AP880705-0018} of DUC) some candidate
    selection methods (see Section~\ref{sec:candidate_extraction}) only select
    ``huge wildfires'' and miss the rightful candidate ``wildfires''. In
    opposition, a relational adjective is a noun substitute that have a
    classificatory or taxonomic meaning~\cite{mcnally2004relationaladjectives},
    which makes it more relevant as a keyphrase modifier.
...
\label{sec:candidate_extraction}
  ...
  that they are always useful. Indeed, relational adjectives have a denominal
  ...
  \label{subsec:keyphrase_extraction}
    ...
    \label{subsubsec:candidate_extraction_result_analysis}
     ...
     \textbf{$~~$9.8} & \textbf{10.0} & \textbf{18.3} & \textbf{12.7}\\
      ...
      produce noise affecting the unsupervised methods. \TODO{Too short.}
...
\label{sec:conclusion}
  ...
  \TODO{A revoir}
  We conducted two experiments to compare the quality of the selected candidates
  ...
  Regarding the results of our method, we showed that adjectives must not
  necessary be keyphrase modifiers and that simple linguistic filters can

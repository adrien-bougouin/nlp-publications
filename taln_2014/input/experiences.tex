...
\label{sec:experiences}
  ...
  \label{subsec:mesure_d_evaluation}
    ...
    effectuées à partir de la racine des mots qui les composent.
  ...
  \label{subsec:resultats}
    ...
    des résultats deux à trois fois inférieurs à ceux obtenus avec les groupes
    ...
    spécificité~\cite{kim2009termextraction}. En opposition, lorsque nous
    comparons les résultats obtenus à partir des candidats termes avec ceux
    obtenus à partir des groupes nominaux pour le corpus d'Archéologie, nous
    observons une légère dégradation des résultats de la méthode TF-IDF et une
    amélioration de ceux de TopicRank. Pour TF-IDF la dégradation des résultats
    est due à un ajout important de candidats composés de déterminants et de
    préposition (environ 50~\% des candidats) alors que les termes-clés de
    référence qui en contiennes sont peut nombreux (3,5~\%). Dans le cas de
    TopicRank, c'est grâce au groupement terminologique de TermSuite que les
    résultats ne sont pas dégradés (MAP à 0,093 en utilisant le groupement par
    défaut de TopicRank).
    %

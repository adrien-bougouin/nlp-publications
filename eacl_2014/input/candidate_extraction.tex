...
\label{sec:section}
  ...
  clustering~\cite{han2007webdocumentclustering}. Since the last decade Internet
  has become the main source of information. Unfortunatly, the huge amount of
  accessible documents are not always associated with keyphrases and the
  enrichment provided by the mentionned tasks cannot be achieved. Therefore, the
  problem of automatically assigning keyphrases to a document is an active field
  of research.
  ...
  n-grams filtered by stop words, NP-chunks or sequences of words matching given
  ...
  to do this by analyzing both reference keyphrases (provided with three
  standard evaluation datasets) and extracted candidates. Also, we investigate
  the feasibility of using term as candidate keyphrases. Terms are TODO \dots
  TODO. Therefore, they seem to be suitable for candidate keyphrase extraction.
  Automatic term detection tend to extract small sets of candidates having
  specified linguistic (e.g. TODO) or statistic properties (e.g. domain
  specificity), whereas the usual methods, such as filtered n-grams, produce a
  huge number of candidates wich may include more candidates matching with
  reference keyphrases, but also many unrelevant candidates. In fact, we compare
  the performance of distinct keyphrase extraction methods (both supervised and
  unsupervised) reguarding the properties of the candidate sets.
  ...
  the methods used to extract candidate keyphrases. Finally,
...
\label{sec:definition_of_candidate_keyphrases}
  ...
  \label{subsec:keyphrase_extraction_datasets}
    ...
    keyphrases given by authors, readers or both. Unlike the methods to
    automatically extract keyphrases, human annotators do not only provide
    ...
    \todo[inline]{[CAPTION] Others are mainly foreign words and coordinating conjonctions.}
    \begin{table*}
      \centering
      \begin{tabular}{rrccc}
        \toprule
  ...
  \label{subsec:keyphrase_analysis}
    ...
    nature,, size, etc.) and their associated keyphrases
    ...
    the fact that the number of n-gram keyphrases (n $\geq$ 3) decreases when n
    increases, means that keyphrases are more often less informative.
    
    \property{Keyphrases bear the minimum information to represent one important
    idea (e.g. TODO instead of TODO).}
  ...
  \label{subsec:keyphrase_extraction}
    ...
    \begin{table*}[h]
      ...
      \begin{tabular}{rccccccccc}
        ...
        \{1..3\}-grams & $~~$7.8 & 10.7 & $~~$8.9 & $~~$9.5 & $~~$6.7 & $~~$7.7 & & & \\

...
\label{sec:section}
  ...
  that best represent the content of the document. Automatic keyphrase
  ...
  tagging (POS tagging). Then the textual units that are assumed to be potential
  keyphrases are extracted as keyphrase candidates. Finally, the keyphrase
  candidates are either classyfied, as ``keyphrase'' or ``non-keyphrase'', or
  ranked, by significance, to select the keyphrases of the document.
  ...
  Another contribution of this article is the testings with terminological
  phrases (terms) as keyphrase candidates. Terms are grammatically arranged
  sequences of words designating a concept and treated as single units. Hence,
  we believe that term candidates are suitable for the keyphrase extraction
  task.
...
\label{sec:keyphrase_extraction}
  ...
  many topics is highly significant and gives more significancy to every topic
  ...
  is the one that appears first into the document.
...
%    \end{table*}
    ...
    \label{subsubsec:method_settings}
      The parameters of methods are chosen to fit as much as possible the
...
%      matching (see above) and assume that they are complex NPs.
    ...
    \label{subsubsec:candidate_extraction_result_analysis}
      ...
      and the other extractions.
      \TODO{talk about quality}

      TermSuite provides a few candidates and do not allow a good maximum
      recall.
      \TODO{but its quality may tackle the problem (see figure 1)}
      \begin{figure*}
...
%        \end{minipage}
  ...
  \label{subsec:keyphrase_extraction}
    ...
    \label{subsubsec:candidate_extraction_result_analysis}
      ...
      pattern matching (learned patterns, longest NPs and NP-chunks).
      \TODO{it is due to the quality}
...
\label{sec:conclusion}
  ...
  candidates is one of the most critical step of the keyphrase extraction.
  ...
  candidate extraction methods and discuss the consistancy of the candidates
  they provide with the inferred properties. Finally, we intrinsically and
  extrinsically compared the keyphrase candidate extraction methods.
  ...
  method that is employed. However, supervised methods seem to be less affected
  by low quality candidate sets.

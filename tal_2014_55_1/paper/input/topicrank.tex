...
\label{sec:extraction_de_termes_cles_avec_topicrank}
  ...
  \label{subsec:identification_des_sujets}
    Dans ce travail, un sujet est une information générale ou très spécifique
    (le plus souvent) véhiculée par au moins une unité textuelle présente dans
    le document analysé. Nous ne nous reposons donc pas sur une formulation très
    ...
    méthode de \newcite{porter1980suffixstripping} afin de considérer identiques
    ...
    la méthode de \newcite{porter1980suffixstripping} (p. ex., les mots
  ...
  \label{subsec:ordonnancement_des_sujets}
    ...
    suivons \newcite{wan2008expandrank} et utilisons la force du lien sémantique
  ...
  \label{subsec:selection_des_termes_cles}
    ...
    termes-clés les plus précis. De ce fait, la stratégie fondée sur le fait
    qu'un sujet est en premier introduit dans sa forme la plus appropriée est la
    stratégie qui nous semble la plus justifiée.

  % Que donne l'extraction ? (exemple)
  La figure~\ref{fig:exemple_topicrank} donne un exemple d'extraction de
  termes-clés avec TopicRank à partir du document d'exemple donné dans la
  figure~\ref{fig:recurrent_example} page~\pageref{fig:recurrent_example}. Dans
  cet exemple, Nous observons un groupement correct de toutes les variantes
  d'\og alertes~\fg, mais aussi un groupement erroné de \og août 2003~\fg\ avec
  ...
  assignés par des humains pour ce document (termes-clés de référence).
  \begin{figure}
    \centering
    \cornersize{0.1}\Ovalbox{
      \parbox{.99\linewidth}{\small
        \textbf{Météo du 19 août 2012~: alerte à la canicule sur la

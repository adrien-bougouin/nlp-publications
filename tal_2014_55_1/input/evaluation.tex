...
%    \end{table}
    ...
    des résultats satisfaisant, nous remarquons qu'il existe encore une marge de
    progression importante. En effet, les valeurs indiquées par la borne haute
    représentent les résultats qui pourraient être obtenue avec une stratégie
    toujours capable de sélectionner un candidat positif. Cette marge de
    progression allant de 4,2 à 19,0 points de f-score est encourageante pour de
    futurs travaux.
  ...
  \label{subsec:analyse_des_sujets_générés}
    Dans cette section, nous analysons les sujets obtenus pour les documents de
    la collection DEFT et tentons de déterminer dans quels cas le groupement
    échoue. Nous observons deux types de cause~: celles qui sont le fruit
    d'erreurs antérieures au groupement en sujets et celles qui sont liées à des
    candidats ayant des propriétés particulières.

    Deux erreurs antérieures au groupement en sujets sont observées. Elles
    concernent toutes les deux l'extraction des termes-clés candidats. En effet,
    ...
    traits~\fg\ groupé avec \og grande ignorance~\fg, etc.). Parmi les expansion
    nominales s'effectuant à droite, les adjectifs relationnels sont moins
    sujets aux erreurs que les autres adjectifs. Il est cependant important de
    noter que lorsque ces adjectifs ont attraits au contexte général du
    document, ils sont très fréquemment utilisés et beaucoup de candidats les
    contenant sont groupés par erreur. Ainsi, dans le document
    ...
    européenne~\fg\ et \og culture européenne~\fg\ sont groupés par erreur parce
    qu'ils partagent tous l'adjectif \og européen~\fg. Outre ces groupements
    ...
    En addition, nous observons des erreurs liées à l'utilisation des radicaux
    lors du calcul de la similarité de Jaccard. Des candidats tels que
    \og empire ottoman~\fg\ et \og définition empirique~\fg sont alors groupés
    du fait que les mots \og empire~\fg\ et \og empirique~\fg\ partagent le
    même radical (\og empir~\fg).

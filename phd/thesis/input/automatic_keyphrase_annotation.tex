...
\label{chap:main-automatic_keyphrase_annotation}
  ...
  \section{Sélection des termes-clés candidats \TODO{pour l'EXTRACTION de
  termes-clés}}
  \label{sec:main-automatic_keyphrase_annotation-keyphrase_candidate_selection}
    ...
    préalablement dans le document. \ANNOTATE{Bien que cette étape préliminaire commence à
    susciter de l'intéret~\cite{wang2014keyphraseextractionpreprocessing}, très
    ...
    définies). Toutefois, la sélection des termes-clés candidats ne doit pas
    être négligée, car c'est cette étape qui détermine la performance maximale
    que peut atteindre une méthode d'indexation par termes-clés. Prenons
    l'exemple de l'approche naïve assurant la performance maximale optimale.
    Pour assurer cette performance maximale optimale, il suffit de fournir
    toutes les séquences possibles de mots présents dans le
    document\footnote{Soit pour chaque phrase de longueur  variable $n$, la
    liste de ses $\{1..n\}$-grammes.}. Cependant, cette approche a pour
    conséquence de sélectionner un nombre considérable de candidats et parmi eux
    un nombre important de candidats négatifs qui augmentent la difficulté de la
    tâche d'indexation par termes-clés~\cite{hasan2014state_of_the_art}.}{c'est
    parti en sucette} Il est
    ...
    \label{subsec:main-automatic_keyphrase_annotation-keyphrase_candidate_selection-analysis_of_keyphrase_properties}
      ...
      \begin{table}[!h]
        ...
        \caption{Rappel des corpus utilisés pour analyser les termes-clés de
                 référence \TODO{segmentation \textsc{Duc}}
      ...
      \begin{table}[!h]
        ...
        \caption{Statistiques concernant les termes-clés de référence des
                 ...
                 \TODO{les PREP et P+D en français sont-elles des variantes
                 d'une forme avec adjectif relationnel ?}
      ...
      \begin{table}[!ht]
        \centering
          \begin{tabular}{l|ccc}
            ...
            \bottomrule
        \end{tabular}
        \caption{Taux d'adjectifs par catégorie (\TODO{rel., qual., attr, comp.})}
      ...
      substantifs\footnote{\TODO{Substantif~: \dots}} (soleil, crâne, cerveau,
      etc.) sans rien changer à leur valeur de substantifs
      --~\cite{bally1944linguistiquegeneraleetlinguistiquefrancaise}\fg{}.
      \TODO{qu'observe-t-on dans le tableau ? $\rightarrow$ s'appuyer sur le
      travail de François Maniez}.
      
      Un adjectif composé est une réunion de mots équivalant à un adjectif, nous
      parlons aussi de locution adjectif (par exemple~:
      \og{}socio-culturel\fg{}). \TODO{et donc ?}. \TODO{qu'observe-t-on dans le
      tableau ?}.
      ...
      au sein d'un terme-clés (\TODO{exemple}). \TODO{moyenmoyen}.
      \TODO{qu'observe-t-on dans le tableau~?}.
      ...
      référence. \TODO{dire qu'il y a ce qu'on veut}
      \begin{table}[!h]
        \centering
        ...
        \caption[
          ...
          Pattrons grammaticaux les plus fréquents parmi les termes-clés
          français et anglais. \TODO{mapping des POS}. \TODO{exemples}.
    ...
    \label{subsec:main-automatic_keyphrase_annotation-keyphrase_candidate_selection-evaluation}
      ...
      \label{subsubsec:main-automatic_keyphrase_annotation-keyphrase_candidate_selection-evaluation-baselines}
        ...
        \textsc{Tf-Idf} et la méthode supervisée \textsc{Kea}. \TODO{pourquoi?}
      ...
      \label{subsubsec:main-automatic_keyphrase_annotation-keyphrase_candidate_selection-evaluation-evaluation_measures}
        ...
        \end{align}
        Plus \TODO{$??$} est élevé, meilleure est la qualité de l'ensemble des
      ...
      \label{subsubsec:main-automatic_keyphrase_annotation-keyphrase_candidate_selection-evaluation-intrinsic_evaluation}
        ...
        avec ceux-ci et la mesure de \TODO{$??$} sont reportés pour chacune des trois
        ...
        candidats sélectionnés et rappel maximum (\TODO{$??$}), notre méthode
  ...
  \label{sec:main-automatic_keyphrase_annotation-unsupervised_automatic_keyphrase_extraction}
    ...
    \label{subsec:main-automatic_keyphrase_annotation-unsupervised_automatic_keyphrase_extraction-evaluation}
      ...
      \label{subsubsec:main-automatic_keyphrase_annotation-unsupervised_automatic_keyphrase_extraction-evaluation-baselines}
        ...
        fondation des méthodes à base de graphe, et la méthode TF-IDF \TODO{qui
        est robiste et encore difficile à battre}.
  ...
  \label{sec:main-automatic_keyphrase_annotation-supervised_automatic_keyphrase_extraction}
    ...
    \label{subsec:main-automatic_keyphrase_annotation-supervised_automatic_keyphrase_annotation-evaluation}
      ...
      \label{subsubsec:main-automatic_keyphrase_annotation-supervised_automatic_keyphrase_annotation-evaluation-comparison}
        ...
        et \textsc{Duc} et plus difficile sur la collection SemEval. \TODO{un
        peu plus de BLABLA et il peut donc s'avérer \dots} Il peut donc s'avérer
        ...
        La figure~\ref{fig:topiccorank-pr_curves} compare le comportement
        respectif de TopicRank, \textit{assignement} et TopicCoRank à l'aide de
        ...
        optimal). Elles aussi montrent un phénomène intéressant~: TopicCoRank à
      ...
      \label{subsubsec:main-automatic_keyphrase_annotation-supervised_automatic_keyphrase_annotation-evaluation-topiccorank-analysis}
        ...
        assignement, avec tout de même une plus forte tendence à l'extraction.
        ...
        TopicCoRank lorsque nous faisons varier la valeur de $\lambda$, qui
        détermine l'influence de la recommendation interne par rapport à la
        recommendation externe, avec un pas de 0,1. Tout d'abord, nous observons
        que la moins bonne performance pouvant être obtenue est bornée par la
        performance de TopicRank. Quelque soit son degré d'influence, le graphe
        du domaine contribue positivement à l'indexation par termes-clés.
        \begin{figure}[h!]
          \centering
          \subfigure[Inist]{
            \begin{tikzpicture}[every axis/.append style={font=\footnotesize}]
        ...
        Selon les collections de données, nous observons deux comportements
        différents. Dans le cas de Termith et de \textsc{Duc} les meilleures
        performances sont obtenues avec une influence plus importante de la
        recommendation externe, tandis que dans le cas de SemEval la
        recommendation interne doit avoir une plus forte influence. Nous
        observons cependant un compromis lorsque $\lambda = 0,5$ (valeur que
        nous avons défini par défaut), soit quand les deux recommendations ont
        le même poids lors de l'ordonnancement.
    ...
    \label{subsec:main-automatic_keyphrase_annotation-supervised_automatic_keyphrase_annotation-conclusion}
      ...
      les méthodes d'extraction et d'assignement. \TODO{trop court}

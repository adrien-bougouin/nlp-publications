      ...
      \begin{table}
        \resizebox{\linewidth}{!}{
          \begin{tabular}{l|c|c|c|c}
            ...
            \hline
            \textsc{Tf-Idf}$^*$ & 27,0 & 36,3 & 6,6 & 12,1\\
        ...
        \caption[
          ...
          Comparaison des méthodes d'extraction automatique de termes-clés de la
          littérature, lorsque dix termes-clés sont extraits. \textsc{Duc} est
...
%        \end{align}
        ...
        \newcite{liu2010topicalpagerank} ont comparé TopicalPageRank à
        \textsc{Tf-Idf} sur les articles journalistiques de
        \newcite{wan2008expandrank}. Contrairement aux précédentes méthodes à
        base de graphe que nous avons présenté, TopicalPageRank améliore
        \textsc{Tf-Idf}. Cette amélioration est significative avec un taux de
        confiance de 95~\%, ce qui signifie que l'amélioration n'est pas due à
        un échantillonage  (le choix des documents pour tester les méthodes)
        favorable à TopicalPageRank.

        ~\\Dans la continuité du travail de \newcite{liu2010topicalpagerank},
        \newcite{zhang2013wordtopicmultirank} ajoutent les sujets de 
        \textsc{Lda} aux n\oe{}uds du graphe de cooccurrences. De cette manière,
        un seul ordonnancement est effectué, qui tient compte de tous les
        sujets. Les auteurs ordonnent conjointement sujets et mots du document
        de sorte que~:
        \begin{itemize}
        ...
        contexte global, la collection de données utilisée pour la modélisation
        des sujets par \textsc{Lda}.
      ...
      \label{subsubsec:main-state_of_the_art-automatic_keyphrase_extraction-unsupervised_keyphrase_extraction-bilan}
        \TODO{\dots}
        \TODO{parler des méthodes à base de graphe et du problème (potentiel) de
        dispersion dans le graphe (adaptation miroir de l'algo de Brin et Page
        pas adaptée dans notre cas)}
    ...
    \label{subsec:main-state_of_the_art-automatic_keyphrase_extraction-conclusion}
      L'extraction automatique de termes-clés est une catégorie d'indexation par
      termes-clés très étudiée. Les méthodes proposées sont nombreuses et
      utilisent des techniques parfois très différentes. Parmi les méthodes non
      supervisées, certaines utilisent des statistiques très simples, tandis que
      d'autres se reposent sur des représentations faisant émerger les relations
      sémantiques entre les termes-clés candidats, ou entre leurs mots. Les
      méthodes supervisées utilisent presque toutes des classificateurs. Elles
      rivalisent entre elles par les traits discriminants qu'elles proposent.
      Parce qu'elles utilisent des données d'entraînement, les méthodes
      supervisées s'adaptent aux types de documents qu'elles doivent traiter et
      sont généralement plus performantes que les méthodes non supervisées.
  ...
  \label{sec:main-state_of_the_art-automatic_keyphrase_assignment}
    ...
    \textsc{Kea}++ double les performances de \textsc{Kea}, sans en augmenter
    ...
    ~\\L'assignement est une catégorie d'indexation par termes-clés encore peu
    étudiée, alors qu'en principe elle attribue des termes-clés d'une qualité
    certifiée par l'usage d'un vocabulaire contrôlé. Seulement deux travaux
    originaux se rapprochent de l'assignement de termes-clés. L'un se fonde sur
    un vocabulaire contrôlé (un thésaurus), mais ne va pas au-delà du contenu du
    document~; l'autre assigne des termes-clés qui n'occurrent pas
    nécessairement dans le document, mais ne se fonde pas sur vocabulaire
    contrôlé ou un quelconque approximation.
  ...
  \label{sec:main-state_of_the_art-automatic_evaluation_of_keyphrase_annotation-conclusion}
    ...
    positifs, et de f-mesure, qui est le compromis entre précision et rappel. Ce
    modèle d'évaluation présente l'avantage d'être utilisable dès lors que des
    données de test sont disponibles. La comparaison à un jugement de référence
    unique rend cependant l'évaluation pessimiste. Lorsque les conditions le
    permettent, une évaluation manuelle reste indispensable pour mieux mesurer
    les forces et les faiblesses d'une méthode d'indexation par termes-clés.

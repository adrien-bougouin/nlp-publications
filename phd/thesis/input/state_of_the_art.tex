...
\label{part:main-state_of_the_art}
  ...
  \label{sec:main-state_of_the_art-introduction}
    ...
    caractérise, mais il s'agit d'une tâche bien distincte de l'indexation plein
    texte de la Recherche d'Information.

    Dans la littérature, nous repérons deux indexations automatiques par
    termes-clés~: l'une est libre, l'autre contrôllée. La première consiste à
    extraire du document les unités textuelles jugées importantes dans son
    contexte. Nous parlons d'extraction automatique de termes-clés. La seconde
    fournit des termes-clés en se fondant sur un vocabulaire contrôllé et sans
    se restreindre aux unités textuelles présentent dans le document. Nous
    parlons d'assignation automatique de termes-clés. En amont de ces
    indexations se trouve aussi une étape récurrente à ne pas négliger~: la
    sélection des termes-clés candidats, c'est-à-dire les unité textuelles qui
    sont potentiellement des termes-clés.

    Dans la suite, nous présentons l'étape de sélection des termes-clés
    candidats, l'extraction automatique de termes-clés, l'assignation
    automatique de termes-clés et, enfin, le processus d'évaluation de
    l'indexation automatique par termes-clés.
  ...
  \label{sec:main-state_of_the_art-keyphrase_candidate_selection}
    % Quel est l'objectif ?
    La sélection des termes-clés candidats\index{Terme-cle candidat@Terme-clé
    candidat \textit{(keyphrase candidate)}|textbf} consiste à déterminer
    ...
    \subsection{Termes-clés candidats classiques}
    \label{subsec:main-state_of_the_art-keyphrase_candidate_selection-keyphrase_candidate_nature}
      ...
      \label{subsubsec:main-state_of_the_art-automatic_keyphrase_extraction-automatic_keyphrase_extraction-pos_sequences}
        ...
        grammaticalement définies\index{Sequence grammaticalement
        definie@Séquence grammaticalement définie \textit{(POS
        sequence)}|textbf} permet de contrôler avec précision la nature et la
    ...
    \label{subsec:main-state_of_the_art-keyphrase_candidate_selection-other_candidates}
      \TODO{voir aussi: extraction
            terminologique~\cite{castellvi2001automatictermdetection}, détection
            de collocation~\cite{pearce2002collocationdetection}, etc.}
  ...
  \label{sec:main-state_of_the_art-automatic_keyphrase_extraction}
    ...
    \label{subsec:main-state_of_the_art-automatic_keyphrase_extraction-unsupervised_keyphrase_extraction}
      ...
      \subsubsection{Les approches statistiques}
  ...
  \label{sec:main-state_of_the_art-automatic_evaluation_of_keyphrase_annotation}
    ...
    \subsection{Mesures de précision, rappel et F-mesure}

...
\label{part:main-state_of_the_art}
  ...
  \label{sec:main-state_of_the_art-keyphrase_candidate_selection}
    ...
    ~\\\TODO{Keyphrase Extraction using Semantic Networks Structure analysis}

    ~\\\TODO{Automatic Keyphrase Extraction from Scientific Documents using
    N-gram Filtration Technique}

    ~\\\newcite{you2009refinedcandidateset} proposent une méthode plus
    sofistiquée que les trois précédentes, leur objectif étant de réduire le
    nombre de candidats sélectionnés sans perdre de candidats positifs ni même
    augmenter la complexité algorithmique de la sélection. \TODO{modifier la
    première phrase car la méthode n'est plus en quatrième position} Leur
    méthode se divise en deux étapes: sélection de candidats préliminaires grâce
    à une liste de mots-clés, puis réduction de cet ensemble de candidats
    ...
    ~\\\TODO{KP-Miner: A Keyphrase Extraction System fo English and Arabic
    Documents}

    ~\\\TODO{Bayesian Text Segmentation for Index Term Identification and

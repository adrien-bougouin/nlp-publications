...
\label{chap:main-data_description}
  ...
  \label{sec:main-data_description-introduction}
    ...
    (\textit{Optical Character Recognition}). Cces procédés ne sont pas
    parfaits, ils ajoutent parfois des fautes d'orthographe (mauvais caractères
    et problèmes d'accents) et ils ne traitent pas toujours correctement les
    ...
    données, la poportion de termes-clés de références n'apparaissant pas dans
    les documents peut être soit très faible, soit très élevée. Dans certains
    cas, les méthodes d'extraction de termes-clés sont donc désavantagées.
    Enfin, la qualification des annotateurs des termes-clés de référence est un
    facteur à ne pas négliger. Nous observons trois annotateurs différents~: les
    auteurs des articles, des indexeurs professionnels et des lecteurs. Les
    auteurs cherchent à attirer le lecteur~; les indexeurs professionnels
    aspirent à une annotation homogène entre tous les documents d'un même
    domaine grâce à un vocabulaire contrôlé~; les lecteurs sont plus libres que
    les auteurs et les indexeurs professionnels. Ils ne sont influencée ni par
    les termes-clés \og{}à la mode\fg{}, ni par un vocabulaire contrôlé.
    ...
    articles journalistiques). Les annotateurs de chaque collection reflètent
  ...
  \label{sec:main-data_description-termith_data}
    ...
    \TODO{définir le méthier d'indexeur}
    
    Les tableaux~\ref{tab:linguistics_journals}~à~\ref{tab:chemestry_journals}
    la répartition revue par revue de chacune des collections. Le projet Termith
    s'intéresse principalement aux sciences humaines et sociales (\textsc{Shs}).
    De ce fait, trois de ses collections de notices représentent une discipline
    de \textsc{Shs}~: linguistique, sciences de l'information et archéologie. La
    quatrième collection représente la chimie. Le corpus de linguistique est
    constitué de 715 notices d'articles français parus entre 2000 et 2012 dans
    12 revues~; le corpus des sciences de l'information contient 706 notices
    d'articles français publiés entre 2001 et 2012 dans six revues~; le corpus
    de psychologie contient 720 notices d'articles français publiés entre 2001
    et 2012 dans sept revues~; le corpus d'archéologie est composé de 718
    notices représentant des articles français parus entre 2001 et 2012 dans 22
    revues~; le corpus de chimie est composé de 782 notices d'articles français
    publiés entre 1983 et 2012 dans quatre revues.
...
%      }
  ...
  \label{sec:main-data_description-deft_data}
    ...
    ensemble de test contenant 93 articles. \TODO{plus de détails sur les
    données du tableau}
    \begin{table}[!h]
      \centering
      \resizebox{\linewidth}{!}{
    ...
    \TODO{point de comparaison KESAKO ???}
  ...
  \label{sec:main-data_description-wikinews_data}
    ...
    Le tableau~\ref{tab:wikinews} donne les détails de ce corpus. \TODO{plus de détails sur les
    données du tableau}
  ...
  \label{sec:main-data_description-semeval_data}
    ...
    100 documents de test. Trois jeux d'annotations sont fournis avec SemEval~:
    les termes-clés des auteurs, les termes-clés de lecteurs et l'union des deux
    ...
    majoritairement extractibles depuis le contenu des articles. \TODO{plus de détails sur les
    données du tableau}
    ...
    La structure des articles est préservée. \TODO{super short}
  ...
  \label{sec:main-data_description-duc_data}
    ...
    figure~\ref{fig:example_duc}). \TODO{plus de détails sur les
    données du tableau}
